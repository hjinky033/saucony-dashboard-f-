...
function updateKPIs() {
    ...
    document.getElementById('kpi-roas').textContent = Math.round(avgROAS) + '%';
    document.getElementById('kpi-cpa').textContent = '₩' + numberFormat(Math.round(avgCPA));
}
    ...
    tbody.innerHTML = tableData.map(item => {
        ...
        return `
            <tr>
                ...
                <td class="number">₩${numberFormat(Math.round(item.cpa))}</td>
                <td class="number ${roasClass}">${Math.round(item.roas)}%</td>
            </tr>
        `;
    }).join('');
